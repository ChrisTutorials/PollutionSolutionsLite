...
local function test_spill_item_stack_factorio_2()
  ...
  -- Test 5: EntityDied loot format (with name/count only, no type field)
  print("\nTest 5: Testing EntityDied loot format (name/count only)...")
  local loot = { name = "red-xenomass", count = 3 }
  local entity_died_success, entity_died_err = pcall(function()
    surface.spill_item_stack({
      position = { x = 10, y = 10 },
      items = { loot },
      force = nil,
  ...
  if entity_died_success then
    print("✓ PASS: EntityDied loot format works (name/count)")
    table.insert(test_results, true)
  else
    print("✗ FAIL: " .. tostring(entity_died_err))
    table.insert(test_results, false)
  end
