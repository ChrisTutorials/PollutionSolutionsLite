...
function fireutil.create_fire_pictures(opts)
  ...
  local fire_flags = {} -- "compressed" flag removed in Factorio 2.0
  local retval = {
    {
...
firetoxic.pictures =
  fireutil.create_fire_pictures({ blend_mode = "normal", animation_speed = 1, scale = 0.5 })
...
firetoxicontree.small_tree_fire_pictures = fireutil.create_small_tree_flame_animations({
  blend_mode = "additive",
  animation_speed = 0.5,
  scale = 0.7 * 0.75,
})
firetoxicontree.pictures = fireutil.create_fire_pictures({
  blend_mode = "additive",
  animation_speed = 1,
  scale = 0.5 * 1.25,
})
...
toxiccloud_medium.action.action_delivery.target_effects.action.action_delivery.target_effects[1].damage.amount =
  12
...
toxiccloud_large.action.action_delivery.target_effects.action.action_delivery.target_effects[1].damage.amount =
  24
